VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} FORM_KALENDER_BEHEREN 
   Caption         =   "Kalender Beheren"
   ClientHeight    =   3615
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   7440
   OleObjectBlob   =   "FORM_KALENDER_BEHEREN.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "FORM_KALENDER_BEHEREN"
...
If IsDate(TextBoxDatum) = True Then
    For Each d In CKalender
        If d.datum = CDate(TextBoxDatum) Then
            If d.ExtraDag <> CheckBox2.Value Then
                d.ExtraDag = CheckBox2.Value
                d.update
                updatekalender
            End If
        End If
...
If IsDate(TextBoxDatum) = True Then
    For Each d In CKalender
        If k.datum = CDate(TextBoxDatum) Then
...
For Each d In Me.CKalender
    If d.datum = datum Then
        If d.feestdag = True Then lbl.BackColor = 255
        aanwezig = d.Zichtbaar
        Exit For
